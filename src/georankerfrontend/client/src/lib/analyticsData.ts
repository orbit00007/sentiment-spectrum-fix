const ANALYTICS_STORAGE_KEY = 'georanker_analytics_data';
...
export const setAnalyticsData = (apiResponse: any) => {
  if (apiResponse && apiResponse.analytics && Array.isArray(apiResponse.analytics)) {
    ...
    try {
      ...
      console.log('📦 [GEORANKER ANALYTICS] Data stored in localStorage');
    } catch (e) {
  ...
  try {
    ...
    if (stored) {
      ...
      if (parsed && parsed.analytics && Array.isArray(parsed.analytics)) {
        ...
        console.log('📦 [GEORANKER ANALYTICS] Data loaded from localStorage');
        return true;
      }
    }
  } catch (e) {
...
export const getModelsUsed = (): string[] => {
  const models = getAnalytics()?.models_used || '';
  return models.split(',').map((m: string) => m.trim()).filter(Boolean);
};

// Get all brands data with formatted logos
...
// Get primary brand data (matching brand_name)
